...
/// 
/// 근접 EnemyStateMachine 과의 유일한 차이: AttackState 가 RangedAttackState.
...
public class RangedEnemyStateMachine : EnemyStateMachineBase
{
    ...
    public RangedAttackState AttackState { get; private set; }
    ...
    protected override void CreateStates()
    {
        ...
        // RangedAttackState 는 RangedEnemyConfig 의 쿨다운 필요.
        // 베이스 _config 는 EnemyConfig 타입이라 안전 캐스팅.
        var rangedConfig = _config as RangedEnemyConfig;
        if (rangedConfig == null)
        {
            Debug.LogError($"[RangedEnemyStateMachine] requires RangedEnemyConfig on {gameObject.name}! " +
                           $"Assign a RangedEnemyConfig asset, not a plain EnemyConfig.");
            // 쿨다운 0 으로 폴백 (작동은 하되 즉시 재발사)
            AttackState = new RangedAttackState(this, 0f);
            return;
        }

        AttackState = new RangedAttackState(this, rangedConfig.AttackCooldown);
    }
    ...
    /// 근접 EnemyStateMachine 과 동일 로직 (미래 베이스 추출 후보).
    /// </summary>
